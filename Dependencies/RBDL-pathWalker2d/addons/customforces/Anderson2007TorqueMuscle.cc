...
#include "Anderson2007TorqueMuscle.h"
#include "../geometry/SmoothSegmentedFunctionFactory.h"
#include "csvtools.h"
...
Passive Torque-Angle Curves  
  ...
  13: k2, 
*/
double const Anderson2007TorqueMuscle::Anderson2007Table3Mean[36][14] = {
...
double const Anderson2007TorqueMuscle::Anderson2007Table3Std[36][14] = {
...
double const Anderson2007TorqueMuscle::GymnastWholeBody[20][14] =
...
Anderson2007TorqueMuscle::Anderson2007TorqueMuscle(
  ...
  muscleName(name)
{
    ...
    strengthScaleFactor      = subjectHeightInMeters*subjectMassInKg*9.81;
    ...
    Anderson2007TorqueMuscle(                  
        ...
        const std::string& name)
            :c1c2c3c4c5c6(c1c2c3c4c5c6),
             b1k1b2k2(b1k1b2k2),
             angleOffset(jointAngleOffsetRelativeToAnderson2007),
             signOfJointAngle(signOfJointAngleRelativeToAnderson2007),
             signOfJointTorque(signOfJointTorque),
             signOfConcentricAnglularVelocity(
                signOfJointTorque),
             muscleName(name)
{
    ...
    /*
        ...
        and
        signOfConcentricAnglularVelocity
        have to be the same, else the sign of joint power and muscle
        power will be inconsistent. Physically inconsistent.
    */
    ...
    //The curve constructors will test that the values
    //of c1 ... c6 make sense. Here we just need to 
    //make sure that all of the values are actually here.
    if(c1c2c3c4c5c6.rows() != 6){
        cerr   << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": c1c2c3c4c5c6 must have 6 elements not "
               << c1c2c3c4c5c6.rows();
        assert(0);
        abort(); 
    }
    ...
    //The curve constructors will test that the values
    //of b1, ... , k2 make sense. Here we just need to 
    //make sure that all of the values are actually here.
    if(b1k1b2k2.rows() != 4){
        cerr << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": b1k1b2k2 must have 4 elements not "
               << b1k1b2k2.rows();
        assert(0);
        abort(); 
    }
     ...
     muscleName(name)
{
    ...
    */
    ...
    if( !(jointHip0Knee1Ankle2==0 || jointHip0Knee1Ankle2==1
                                  || jointHip0Knee1Ankle2==2) ){
        cerr  << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": jointHip0Knee1Ankle2 must be [0,1,2] "
               << jointHip0Knee1Ankle2;
        assert(0);
        abort();
    }

    if( !(directionExtension0Flexion1==0 || directionExtension0Flexion1==1) ){
        cerr << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": directionExtension0Flexion1 must be [0,1] "
               << directionExtension0Flexion1;
        assert(0);
        abort();
    }

    if( !(genderMale0Female1==0 || genderMale0Female1==1) ){
        cerr << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": genderMale0Female1 must be [0,1] "
               << genderMale0Female1;
        assert(0);
        abort();
    }

    if( !(ageYoung0Mid1Senior2==0 || ageYoung0Mid1Senior2==1
                                  || ageYoung0Mid1Senior2==2) ){
        cerr   << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << ": ageYoung0Mid1Senior2 must be [0,1] "
               << ageYoung0Mid1Senior2;
        assert(0);
        abort();
    }
    ...
    for(int i=0; i<36;++i){
        ...
        if( abs(Anderson2007Table3Mean[i][jointIdx]
                  -(double)jointHip0Knee1Ankle2) < EPSILON
         && abs(Anderson2007Table3Mean[i][dirIdx]
                   -(double)directionExtension0Flexion1) < EPSILON
         && abs(Anderson2007Table3Mean[i][genderIdx]
                   -(double)genderMale0Female1) < EPSILON
         && abs(Anderson2007Table3Mean[i][ageIdx]
                   -(double)ageYoung0Mid1Senior2) < EPSILON){
            idx = i;
        }
    }
    ...
    if(idx == -1){
        cerr << "Anderson2007TorqueMuscle::"
               << "Anderson2007TorqueMuscle:"
               << muscleName
               << "The joint, direction, gender, and age indices "
               << "could not be found in Table 3 of Anderson 2007 et al.";
        assert(0);
        abort();
    }
...
double Anderson2007TorqueMuscle::
                        ...
                        double activation) const 
{
  ...
  if(muscleCurvesAreDirty){
    Anderson2007TorqueMuscle* mutableThis =
        const_cast<Anderson2007TorqueMuscle* >(this);
...
const RigidBodyDynamics::Math::VectorNd&
    Anderson2007TorqueMuscle::getParametersC1C2C3C4C5C6()
{
    return c1c2c3c4c5c6;
}

const RigidBodyDynamics::Math::VectorNd&
    Anderson2007TorqueMuscle::getParametersB1K1B2K2()
{
    return b1k1b2k2;
}


const SmoothSegmentedFunction& Anderson2007TorqueMuscle::
...
string Anderson2007TorqueMuscle::getName(){
  return muscleName;
}

void Anderson2007TorqueMuscle::setName(string &name){
  muscleCurvesAreDirty = true;
  muscleName = name;
}
...
void Anderson2007TorqueMuscle::updateTorqueMuscleCurves()
{
...
void Anderson2007TorqueMuscle::printJointTorqueProfileToFile(
