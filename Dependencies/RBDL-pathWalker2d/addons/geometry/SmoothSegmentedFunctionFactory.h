#ifndef SMOOTHSEGMENTEDFUNCTIONFACTORY_H_
#define SMOOTHSEGMENTEDFUNCTIONFACTORY_H_
/* -------------------------------------------------------------------------- *
...
static void createAnderson2007ActiveTorqueAngleCurve(
  ...
  */
  static void createAnderson2007PassiveTorqueAngleCurve(
      ...
      SmoothSegmentedFunction& smoothSegmentedFunctionToUpdate); 
  /**
  This function constructs the nonlinear angle-torque profile associated
  with the NeuroSwing ankle orthoses. This profile looks something like
  this
  ...
  Note that the ASCII diagram is necessary simplified:
  k1 and k3 are not 0 nor necessarily equal.
  k0, k2, and k4 are not necessarily equal.

  At the moment this code is being written it is not clear how this
  profile varies from one spring to the next, and so the interface
  that is made for this curve does little more than provide a 
  convenience method to build the above curve given a 3 vectors:
  a vector of x control points in degrees
  a vector of y control points in Nm/kg
  a vector of slopes in (Nm/kg)/degrees

  The above curve was extracted from a NeuroSwing that is attached
  to a carbon fiber foot and lower leg. Each of the above regions
  has the following physical interpretation:

  Line Segment

  Dorsi-flexion support
  0:  The NeuroSwing joint has hit a hard stop. The large stiffness
  here is due to the stiffness of the carbon fiber foot.

  1:  One of the 2 Neuroswing springs is providing a torque in dorsiflexion.

  Neutral.
  2:  The offset angle has been reached, and one spring is unloading
  while the other spring is being loaded. Due to the preload of
  the two springs there is a gap in torque

  Plantar-flexion support
  3:  The other Neuroswing spring is providing a torque in plantar flexion.

  4:  The Neuroswing joint has hit a hardstop in plantar flexion. The
  observed stiffness is due to the stifness of the carbon fiber limb

   @param controlPointAngles:
    A vector of the angles of the 5 control points in order
    [x0, x1, x2, x3, x4] (radians)

   @param controlPointTorques:
    A vector of the torques at the 5 control points locations in order
    [y0, y1, y2, y3, y4] (Nm)

   @param controlPointSlopes:
    A vector of the stiffesses of the line segments at the control
    point locations
    [k0, k1, k2, k3, k4] (Nm/rad)
  ...
  */
  static void createNeuroSwingSpringProfile(  
...
#endif //SMOOTHSEGMENTEDFUNCTIONFACTORY_H_
