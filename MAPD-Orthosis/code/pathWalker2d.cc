...
WalkerModel walker;
const char* const model_file = "orthosis.lua";
...
void LoadModelAndConstraints () {
	...
	if (verbose)
	  cout << "[LoadModelAndConstraints] Loading Orthosis Parameters" <<  endl;
	if (!walker.loadOrthosisParams (model_file, verbose))
	  abort();
...
static int rdfcn_right_flat_s_n = 12, rdfcn_right_flat_s_ne = 8;
void rdfcn_right_flat_s(double *ts, double *sd, double *sa, double *u, double *p, double *pr, double *res, long *dpnd, InfoPtr *info) {
  ...
  // Maintain Right Halx vertical velocity to zero
  ...
  res[8] = RightHeelForce[2];
  res[9] = RightHalxForce[2];
  
  // Maintain anterior-posterior forces at Right Heel within friction cone
  res[10] = (limitingFriction-RightHeelForce[0]);
  res[11] = (limitingFriction+RightHeelForce[0]);
}
...
vector<VectorNd> orthosis_torque_values;
...
void hires_data_out(double *t, double *sd, double *sa, double *u, double *p, double *rwh, long *iwh, InfoPtr *info) {
	...
	if (t_values.size() > 0) {
	  for (unsigned int i = 0; i < t_values.size(); i ++) {
	    ...
	    // Write orthosis torque values
	    for (unsigned int j = 0; j < 2; j++) {
	      augmented_data_stream << orthosis_torque_values[i][j];
	      if (j < orthosis_torque_values[i].size() -1 )
		augmented_data_stream << ", ";
	    }
    ...
    VectorNd orthosis_torque_vec (2);
    orthosis_torque_vec[0] = walker.orthosis_r.calcJointTorque (sd[StateRightAnkleRotY]);
    orthosis_torque_vec[1] = walker.orthosis_l.calcJointTorque (sd[StateLeftAnkleRotY]);
    orthosis_torque_values.push_back(orthosis_torque_vec);
...
void def_model(void) {
  ...
  def_mstage(3, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_right_halx_left_flat, NULL, NULL, NULL);
  
  // right_flat
  def_mstage(4, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_left_flat, NULL, NULL, NULL);
  
  // left_halx
  def_mstage(5, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_left_halx, NULL, NULL, NULL);

  // left halx right heel
  def_mstage(6, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_left_halx_right_heel, NULL, NULL, NULL);

  // right halx left flat
  def_mstage(7, nxd, nxa, nu, mfcn, NULL, 0, 0, 0, NULL, ffcn_left_halx_right_flat, NULL, NULL, NULL);
  ...
  def_mpc(7, "e", npr, NULL, NULL, NULL, rcfcn_limit_cycle_e);
