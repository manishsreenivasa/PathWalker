...
const unsigned int MayerStatesNum = 1;

WalkerModel walker;
const char* const model_file = "barefeet_healthy.lua";
...
{ 
  ...
  sd[MayerStateActivationSquared] = 0.;
}
...
void mfcn(double *t, double *sd, double *sa, double *p, double *pr, double *mval, long *dpnd, InfoPtr *info) {
  ...
  double mfcn_scale = 1.0;
   
  *mval = 0.;
  *mval = sd[MayerStateActivationSquared] / (1e-6 + fabs(sd[StatePelvisPosX])) * mfcn_scale;
}
...
void ffcn_right_flat (double *t, double *xd, double *xa, double *u, double *p, double *rhs, double *rwh, long *iwh, InfoPtr *info) {
  ...
  ffcn_cost(t, xd, xa, u, p, rhs, rwh, iwh, info);
}
...
static int rdfcn_right_halx_left_flat_e_n = 9, rdfcn_right_halx_left_flat_e_ne = 5;
void rdfcn_right_halx_left_flat_e(double *ts, double *sd, double *sa, double *u, double *p, double *pr, double *res, long *dpnd, InfoPtr *info) {
  if (*dpnd) {
  ...
  res[2] = RightHalxPos[2];
  res[3] = RightHalxForce[2];  
  res[4] = RightHalxForce[0];
  
  // Maintain positive forces on Left Halx and Left Heel
  res[5] = LeftHalxForce[2];
  res[6] = LeftHeelForce[2];  
...
static void rcfcn_limit_cycle_s (double *ts, double *sd, double *sa, double *u, double *p, double *pr, double *res, long *dpnd, InfoPtr *info) {
  ...
  res[ 0] = sd[ StateRightHipRotY              ];
  res[ 1] = sd[ StateRightKneeRotY             ];
  res[ 2] = sd[ StateRightAnkleRotY            ];
  res[ 3] = sd[ StateLeftHipRotY               ];
  res[ 4] = sd[ StateLeftKneeRotY              ];
  res[ 5] = sd[ StateLeftAnkleRotY             ];         
  res[ 6] = sd[ StateTorsoRotY                 ];             
  res[ 7] = sd[ StateRightHipRotVelY           ];                   
  res[ 8] = sd[ StateRightKneeRotVelY          ];                    
  res[ 9] = sd[ StateRightAnkleRotVelY         ];  
  res[10] = sd[ StateLeftHipRotVelY            ];                   
  res[11] = sd[ StateLeftKneeRotVelY           ];                    
  res[12] = sd[ StateLeftAnkleRotVelY          ];      
  res[13] = sd[ StateTorsoRotVelY              ];     
  res[14] = sd[ StateRightHipExtensionRotActY  ];                            
  res[15] = sd[ StateRightHipFlexionRotActY    ];                          
  res[16] = sd[ StateRightKneeExtensionRotActY ];                             
  res[17] = sd[ StateRightKneeFlexionRotActY   ];                           
  res[18] = sd[ StateRightAnkleExtensionRotActY];                              
  res[19] = sd[ StateRightAnkleFlexionRotActY  ];                            
  res[20] = sd[ StateLeftHipExtensionRotActY   ];                           
  res[21] = sd[ StateLeftHipFlexionRotActY     ];                         
  res[22] = sd[ StateLeftKneeExtensionRotActY  ];                            
  res[23] = sd[ StateLeftKneeFlexionRotActY    ];                          
  res[24] = sd[ StateLeftAnkleExtensionRotActY ];                             
  res[25] = sd[ StateLeftAnkleFlexionRotActY   ];                           
  res[26] = sd[ StateTorsoExtensionRotActY     ];                         
  res[27] = sd[ StateTorsoFlexionRotActY       ]; 
  res[28] = u[ControlRightHipExtensionRotY  ];                            
  res[29] = u[ControlRightHipFlexionRotY    ];                          
  res[30] = u[ControlRightKneeExtensionRotY ];                             
  res[31] = u[ControlRightKneeFlexionRotY   ];                           
  res[32] = u[ControlRightAnkleExtensionRotY];                              
  res[33] = u[ControlRightAnkleFlexionRotY  ];                            
  res[34] = u[ControlLeftHipExtensionRotY   ];                           
  res[35] = u[ControlLeftHipFlexionRotY     ];                         
  res[36] = u[ControlLeftKneeExtensionRotY  ];                            
  res[37] = u[ControlLeftKneeFlexionRotY    ];                          
  res[38] = u[ControlLeftAnkleExtensionRotY ];                             
  res[39] = u[ControlLeftAnkleFlexionRotY   ];                           
  res[40] = u[ControlTorsoExtensionRotY     ];                         
  res[41] = u[ControlTorsoFlexionRotY       ];
  res[42] = sd[ StatePelvisRotVelY          ];
  res[43] = sd[ StatePelvisVelZ             ];
  res[44] = sd[ StatePelvisVelX             ];
  res[45] = sd[ StatePelvisRotY             ];  
}

static void rcfcn_limit_cycle_e (double *ts, double *sd, double *sa, double *u, double *p, double *pr, double *res, long *dpnd, InfoPtr *info) {
  if (*dpnd) {
    *dpnd = RFCN_DPND(0, *sd, *u, 0, 0, 0);
    return;
  }

  res[ 0] = -sd[ StateLeftHipRotY              ];
  res[ 1] = -sd[ StateLeftKneeRotY             ];
  res[ 2] = -sd[ StateLeftAnkleRotY            ];           
  res[ 3] = -sd[ StateRightHipRotY             ];
  res[ 4] = -sd[ StateRightKneeRotY            ];
  res[ 5] = -sd[ StateRightAnkleRotY           ];      
  res[ 6] = -sd[ StateTorsoRotY                ];      
  res[ 7] = -sd[ StateLeftHipRotVelY            ];                   
  res[ 8] = -sd[ StateLeftKneeRotVelY           ];                    
  res[ 9] = -sd[ StateLeftAnkleRotVelY          ];     
  res[10] = -sd[ StateRightHipRotVelY           ];                   
  res[11] = -sd[ StateRightKneeRotVelY          ];                    
  res[12] = -sd[ StateRightAnkleRotVelY         ];       
  res[13] = -sd[ StateTorsoRotVelY              ];                  
  res[14] = -sd[ StateRightHipExtensionRotActY  ];                            
  res[15] = -sd[ StateRightHipFlexionRotActY    ];                          
  res[16] = -sd[ StateRightKneeExtensionRotActY ];                             
...
void hires_data_out(double *t, double *sd, double *sa, double *u, double *p, double *rwh, long *iwh, InfoPtr *info) {
	...
	if (t_values.size() > 0) {
	  for (unsigned int i = 0; i < t_values.size(); i ++) {
	    ...
	    for (unsigned int j = NoOfPos; j < sd_values[i].size()-1; j++) {
	      augmented_data_stream << sd_values[i][j];
	      if (j < sd_values[i].size() -2 )
...
void def_model(void) {
  ...
  nrc   = StateNameLast + ControlNameLast - 1 - MayerStatesNum;
  nrce  = StateNameLast + ControlNameLast - 1 - MayerStatesNum;
  
  // Define OCP dimensions
  def_mdims(nmos, np, nrc, nrce);

  // right_flat
  def_mstage(0, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_right_flat, NULL, NULL, NULL);
  
  // right_halx
  def_mstage(1, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_right_halx, NULL, NULL, NULL);

  // right halx left heel
  def_mstage(2, nxd, nxa, nu, NULL, NULL, 0, 0, 0, NULL, ffcn_right_halx_left_heel, NULL, NULL, NULL);

  // right halx left flat
  def_mstage(3, nxd, nxa, nu, mfcn, NULL, 0, 0, 0, NULL, ffcn_right_halx_left_flat, NULL, NULL, NULL);
  
  // Define Constraints for OCP
  def_mpc(0, "s", npr, rdfcn_right_flat_s_n, rdfcn_right_flat_s_ne, rdfcn_right_flat_s, rcfcn_limit_cycle_s);
  ...
  def_mpc(3, "e", npr, rdfcn_right_halx_left_flat_e_n, rdfcn_right_halx_left_flat_e_ne, rdfcn_right_halx_left_flat_e, rcfcn_limit_cycle_e);
